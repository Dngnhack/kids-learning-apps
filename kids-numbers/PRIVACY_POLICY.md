# Privacy Policy — DL Numbers (Count & Learn)

**Publisher:** Digital Legends LLC (Texas) · **Contact:** randy@digital-legends-llc.com
**App:** DL Numbers — Count & Learn

> **Plain-language summary:** This app collects **nothing**. It works fully offline, shows **no ads**, and sends **no data** anywhere.

## What we collect
**None.** DL Numbers does **not** collect, store off-device, or transmit any personal information. It does **not** use analytics, advertising, tracking, accounts, or third-party SDKs.

## On-device data
The app saves your child's **learning progress** (which numbers they've practiced) **only on this device**, using local storage. This never leaves the device and is not personal information. You can erase it anytime: **Grown-ups → Reset progress**, or by uninstalling the app.

## Children's privacy (COPPA)
DL Numbers is designed for young children and is built to be **COPPA-compliant by design**: because we collect no personal information and transmit nothing, there is nothing to consent to share. We do not knowingly collect personal information from anyone, including children under 13.

## Permissions
The app requests **no sensitive permissions** (no location, no contacts, no camera/microphone, no device identifiers). It uses your device's built-in text-to-speech for audio, processed on-device.

## Ads & purchases
**No ads. No tracking. No subscriptions.**

## Changes
If this policy changes, the updated version will be posted here.

## Contact
Questions: **randy@digital-legends-llc.com**.
